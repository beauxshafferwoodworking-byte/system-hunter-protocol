const STORE_KEY = 'system_hunter_protocol_v16';
...
const ACHIEVEMENTS = [
  { id:'first_hunt', name:'First Hunt', tier:'Common', desc:'Complete your first full Daily Quest.', progress:s=>s.fullCompletions, target:1, title:'First Hunter' },
  { id:'persistence_i', name:'Persistence I', tier:'Common', desc:'Reach a 3-day streak.', progress:s=>s.streak, target:3, title:'Persistent Survivor' },
  { id:'persistence_ii', name:'Persistence II', tier:'Rare', desc:'Reach a 7-day streak.', progress:s=>s.streak, target:7, title:'System Disciple' },
  { id:'iron_spine', name:'Iron Spine', tier:'Rare', desc:'Complete 5 recovery-focused quests.', progress:s=>s.recoveryCompletions, target:5, title:'Iron Spine' },
  { id:'conditioned', name:'Conditioning Candidate', tier:'Rare', desc:'Complete 5 endurance or conditioning quests.', progress:s=>s.enduranceCompletions, target:5, title:'Conditioning Candidate' },
  { id:'strength_seed', name:'Strength Seed', tier:'Common', desc:'Complete 5 strength or hybrid quests.', progress:s=>s.strengthCompletions, target:5, title:'Strength Seed' },
  { id:'no_excuses', name:'No Excuses', tier:'Epic', desc:'Complete a quest while fatigue is High.', progress:s=>s.highFatigueCompletions, target:1, title:'No Excuses' },
  { id:'awakening', name:'Awakening', tier:'Epic', desc:'Reach Level 5.', progress:s=>s.level, target:5, title:'Awakened Hunter' }
];

const baseState = {
  level:1, xp:0, streak:0, missed:0,
  stats:{ str:1, end:1, agi:1, vit:1, rec:1 },
  title:'Exhausted Survivor', rank:'E-RANK', dayType:'Awaiting Input', quests:[], log:['SYSTEM initialized. Awaiting Hunter input.'],
  fullCompletions:0, partialCompletions:0, recoveryCompletions:0, enduranceCompletions:0, strengthCompletions:0, highFatigueCompletions:0,
  unlockedAchievements:[]
};
let state = { ...baseState, ...(JSON.parse(localStorage.getItem(STORE_KEY) || 'null') || {}) };
...
function currentRank(){ return state.level >= 14 ? 'C-RANK' : state.level >= 7 ? 'D-RANK' : 'E-RANK'; }
function showOverlay(title, body, eyebrow='SYSTEM ALERT'){
  if(!$('systemOverlay')) return;
  $('overlayEyebrow').textContent = eyebrow; $('overlayTitle').textContent = title; $('overlayBody').textContent = body; $('systemOverlay').classList.remove('hidden');
}
...
function generateQuest(){
  const i = getInputs(); const recoveryMode = i.fatigue >= 4 || i.sleep <= 2 || i.backPain >= 3 || i.neckPain >= 3; const day = state.streak % 6;
  let quests = []; let dayType = recoveryMode ? 'Recovery Gate' : ['Strength','Endurance','Mobility','Strength','Conditioning','Hybrid'][day];
  ...
  if(recoveryMode){ quests.push({ name:'Core Stabilization', detail:'Dead bug 3x8/side, glute bridge 3x12, bird dog 3x8/side.', done:false },{ name:'Neck Recovery Protocol', detail:'Chin tucks 3x8, gentle rotations x8/side, scapular retractions 3x12.', done:false },{ name:'Zone 2 Walk', detail:'20-30 min easy walk. Nose breathing if possible.', done:false }); }
  else if(dayType === 'Strength'){ quests.push({ name:'Push Progression', detail:`Pushups ${Math.min(5 + state.level, 10)} sets of 8-15. Stop 2 reps before failure.`, done:false },{ name:'Leg Foundation', detail:'Air squats 4x15, reverse lunges 3x8/side, calf raises 3x20.', done:false },{ name:'Core Armor', detail:'Plank 3x35-60 sec, dead bug 3x8/side, hollow hold 3x15-25 sec.', done:false }); }
  else if(dayType === 'Endurance' || dayType === 'Conditioning'){ quests.push({ name:'Run-Walk Conditioning', detail:'5 min warmup, then 8 rounds: 60 sec jog + 90 sec walk.', done:false },{ name:'Durability Circuit', detail:'3 rounds: 12 squats, 8 pushups, 20 mountain climbers.', done:false },{ name:'Breathing Reset', detail:'3 minutes nasal breathing while lying on back with feet elevated.', done:false }); }
  else if(dayType === 'Mobility'){ quests.push({ name:'Spine Decompression', detail:'Child’s pose breathing, couch stretch, hamstring flossing, open books.', done:false },{ name:'Posture Repair', detail:'Wall slides 3x8, scapular pushups 3x10, chin tucks 3x8.', done:false },{ name:'Easy Walk', detail:'20-30 min easy walk. No max effort today.', done:false }); }
  else { quests.push({ name:'Hybrid Bodyweight Circuit', detail:'4 rounds: 10 pushups, 15 squats, 8 lunges/side, 30 sec plank.', done:false },{ name:'Pull-Up Bonus', detail:'If a bar is available: 5 sets of 4-6 pullups, clean reps only.', done:false },{ name:'Recovery Protocol', detail:'Hip flexor stretch, thoracic rotations, neck reset, 5-8 minutes.', done:false }); }
...
function checkAchievements(){
  const unlockedNow = [];
  ACHIEVEMENTS.forEach(a => { if(!state.unlockedAchievements.includes(a.id) && a.progress(state) >= a.target){ state.unlockedAchievements.push(a.id); unlockedNow.push(a); } });
  if(unlockedNow.length){ const a = unlockedNow[0]; state.title = a.title; addLog(`Achievement unlocked: ${a.name}. Title acquired: ${a.title}.`); showOverlay('TITLE UNLOCKED', `${a.name} — ${a.title}`, 'ACHIEVEMENT'); }
}
function completeDay(){
  if(!state.quests.length) generateQuest(); const completed = state.quests.filter(q=>q.done).length; const total = state.quests.length;
  if(completed === 0){ showOverlay('SYSTEM WARNING','No objectives completed. Check off at least one quest objective first.'); return; }
  const input = getInputs(); const full = completed === total; const gain = completed * 45 + (full ? 80 : 0);
  state.xp += gain; state.stats.vit += 1;
  if(full){ state.streak += 1; state.fullCompletions += 1; } else { state.partialCompletions += 1; }
  if(full && input.fatigue >= 4) state.highFatigueCompletions += 1;
  if(state.dayType.includes('Strength') || state.dayType.includes('Hybrid')){ state.stats.str += 1; if(full) state.strengthCompletions += 1; }
  if(state.dayType.includes('Endurance') || state.dayType.includes('Conditioning')){ state.stats.end += 1; if(full) state.enduranceCompletions += 1; }
  if(state.dayType.includes('Mobility')) state.stats.agi += 1;
  if(state.dayType.includes('Recovery') || state.dayType.includes('Gate')){ state.stats.rec += 1; if(full) state.recoveryCompletions += 1; }
  while(state.xp >= nextXp()){ state.xp -= nextXp(); state.level += 1; showOverlay('LEVEL UP',`Hunter level increased to ${state.level}.`,'RANK UP'); }
  state.rank = currentRank(); addLog(`${full ? 'Daily Quest complete' : 'Partial quest submitted'}. +${gain} XP.`); if(full) state.quests = state.quests.map(q=>({...q, locked:true})); checkAchievements(); save();
}
function missDay(){ state.streak = 0; state.missed += 1; state.xp = Math.max(0, state.xp - 35); state.dayType = 'Penalty Active'; state.quests = [{ name:'Penalty Quest: Restoration', detail:'10 min mobility reset, wall sit, dead bugs, bird dogs, easy breathing.', done:false },{ name:'System Warning', detail:'No ego lifting tomorrow. Rebuild consistency.', done:false }]; addLog('Penalty Quest activated. Streak reset. -35 XP.'); showOverlay('PENALTY QUEST','Hunter consistency failure detected.','SYSTEM WARNING'); save(); }
function render(){
  $('level').textContent = state.level; $('xp').textContent = state.xp; $('nextXp').textContent = nextXp(); $('xpFill').style.width = `${Math.min(100,(state.xp/nextXp())*100)}%`; $('streak').textContent = state.streak; $('rank').textContent = currentRank(); $('title').textContent = state.title; $('dayType').textContent = state.dayType;
  ...
  const completed = state.quests.filter(q=>q.done).length; const total = state.quests.length; $('questProgressText').textContent = `${completed}/${total} objectives complete`; $('questProgressFill').style.width = `${total ? (completed/total)*100 : 0}%`;
  ...
  if($('achievementList')) $('achievementList').innerHTML = ACHIEVEMENTS.map(a=>{ const value = Math.min(a.progress(state), a.target); const unlocked = state.unlockedAchievements.includes(a.id); return `<div class="achievement ${unlocked?'':'locked'}"><strong>${unlocked?'◆':'◇'} ${a.name}</strong><small>${a.tier} — ${a.desc}<br>${value}/${a.target} complete${unlocked?`<br>Title: ${a.title}`:''}</small></div>`; }).join('');
  ...
  $('penaltyText').textContent = state.dayType === 'Penalty Active' ? 'Penalty active. Complete restoration quest to stabilize the system.' : 'No penalty active. Maintain your streak, Hunter.'; $('log').innerHTML = state.log.map(item=>`<p>${item}</p>`).join('');
}
...
if($('overlayClose')) $('overlayClose').addEventListener('click',()=>$('systemOverlay').classList.add('hidden')); if($('codexClose')) $('codexClose').addEventListener('click',()=>$('codexOverlay').classList.add('hidden'));
